...
function WorkerRow({ worker, handleDeletePerson, handleEditPerson }) {
  ...
  const workerObject = new WorkerPerson(
    id,
    name,
    surname,
    lastname,
  ...
  return (
    <tr className="worker" key={worker.id}>
      ...
      <td>{workerObject.workPeriod} year(s)</td>
      <td>{workerObject.post}</td>
      <td>
        <EditButton id={worker.id} />
        <button
          className="btn btn-danger"
          type="button"
          onClick={() => handleDeletePerson(worker.id)}
        >
          <svg
